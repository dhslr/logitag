(function () {
	...
	Concast = function () {
		...
		timeframe.calc = function (sub_tree) {
			if (_.isArray(sub_tree) && sub_tree.length > 0) {
				switch (sub_tree[0]) {
					case '&&':
						return 	Math.max(timeframe.calc(sub_tree[1][0]), timeframe.calc(sub_tree[1][1]));
					case '||':
						return  Math.max(timeframe.calc(sub_tree[1][0]), timeframe.calc(sub_tree[1][1]));
					case '->':
						return Number(sub_tree[1][0]);
					default:
						return 0;	
				}
			} else {
		...
		update_tags = function () {
			//console.log("appool_url: %j", appool_url);
